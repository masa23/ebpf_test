...
func main() {
	...
	if err := netlink.QdiscAdd(qdisc); err != nil {
		log.Printf("cannot add clsact qdisc: %v", err)
		if err := netlink.QdiscDel(qdisc); err != nil {
			log.Fatalf("cannot del clsact qdisc: %v", err)
		}
		if err := netlink.QdiscAdd(qdisc); err != nil {
	...
	go func() {
		for {
			key := make([]byte, 16)
			var count uint64
			m4 := objs.tcMaps.Inet4Map
			m6 := objs.tcMaps.Inet6Map
			err := m4.NextKey(nil, &key)
			if err != nil {
				time.Sleep(time.Second)
				continue
			}
			for i := 0; i <= int(m4.MaxEntries()); i++ {
				err = m4.Lookup(key, &count)
				if err != nil {
					fmt.Printf("count: %d\n", i+1)
					break
				}
			...
			err = m6.NextKey(nil, &key)
			if err != nil {
				continue
			}
			for i := 0; i <= int(m6.MaxEntries()); i++ {
				err = m6.Lookup(key, &count)
				if err != nil {
					fmt.Printf("count: %d\n", i+1)
					break
				}
